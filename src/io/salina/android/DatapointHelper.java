...
public final class DatapointHelper
{
    ...
    /* package */static int getApiLevel()
    {
        ...
        {
            Log.w(Config.LOG_TAG, "Caught exception", e); //$NON-NLS-1$
            ...
            catch (final Exception ignore)
            {
                ...
                {
                    Log.w(Config.LOG_TAG, "Caught exception", ignore); //$NON-NLS-1$
                }
            }
        }
    ...
    public static String getTelephonyDeviceIdOrNull(final Context context)
    {
        if (Config.CURRENT_API_LEVEL >= 7)
        {
            ...
            if (!hasTelephony.booleanValue())
            {
                if (Config.IS_LOGGABLE)
                {
                    Log.i(Config.LOG_TAG, "Device does not have telephony; cannot read telephony id"); //$NON-NLS-1$
                }
        ...
        else
        {
            if (Config.IS_LOGGABLE)
            {
                Log.w(Config.LOG_TAG, "Application does not have permission READ_PHONE_STATE; determining device id is not possible.  Please consider requesting READ_PHONE_STATE in the AndroidManifest"); //$NON-NLS-1$
            }
        }
    ...
    public static String getWifiMacHashOrNull(final Context context)
    {
        if (Config.CURRENT_API_LEVEL >= 8)
        {
            ...
            if (!hasWifi.booleanValue())
            {
                if (Config.IS_LOGGABLE)
                {
                    Log.i(Config.LOG_TAG, "Device does not have Wi-Fi; cannot read Wi-Fi MAC"); //$NON-NLS-1$
                }
        ...
        else
        {
            if (Config.IS_LOGGABLE)
            {
                 ...
                 */
                Log.i(Config.LOG_TAG, "Application does not have permission ACCESS_WIFI_STATE; determining MAC address is not possible."); //$NON-NLS-1$
            }
        }
    ...
    public static String getNetworkType(final Context context, final TelephonyManager telephonyManager)
    {
        try
        {
            ...
            else
            {
                if (Config.IS_LOGGABLE)
                {
                    Log.w(Config.LOG_TAG, "Application does not have one more more of the following permissions: ACCESS_WIFI_STATE. Determining Wi-Fi connectivity is unavailable"); //$NON-NLS-1$
                }
            }
        }
        catch (final SecurityException e)
        {
            ...
            if (Config.IS_LOGGABLE)
            {
                Log.w(Config.LOG_TAG, "Application does not have the permission ACCESS_NETWORK_STATE. Determining Wi-Fi connectivity is unavailable", e); //$NON-NLS-1$
            }
        }
    ...
    public static String getAppVersion(final Context context)
    {
        ...
        try
        {
            ...
            if (null == versionName)
            {
                if (Config.IS_LOGGABLE)
                {
                    Log.w(Config.LOG_TAG, "versionName was null--is a versionName attribute set in the Android Manifest?"); //$NON-NLS-1$
                }
